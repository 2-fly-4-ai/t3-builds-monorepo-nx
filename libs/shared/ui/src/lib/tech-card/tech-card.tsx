...
export function TechCard(props: TechCardProps) {
  ...
  const isPostModalOpen = posts.includes(props.post.id);
  ...
  }, [props.post.id, isPostModalOpen, togglePosts]);
    ...
    <div className="group grid  grid-cols-10 gap-2 gap-x-8 rounded-xl border-2 bg-white  bg-opacity-10 p-4 py-4 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] transition duration-200 hover:shadow-[0px_0px_5px_5px_rgb(231,229,228)] dark:border-gray-300">
      <div className="relative col-span-full  rounded-none ">
        <div className="group  absolute flex h-full w-full transition duration-500 group-hover:bg-black group-hover:bg-opacity-20">
          ...
          <button
            ...
            className="mx-auto mx-auto my-auto mt-4 hidden items-center justify-center  gap-2  rounded-lg border-4  px-2 py-1 text-base font-bold antialiased backdrop-blur transition duration-500 group-hover:flex group-hover:bg-white  group-hover:bg-opacity-80 dark:group-hover:bg-black dark:group-hover:bg-opacity-50"
          >
            VIEW ARTICLE
            <svg
              stroke="currentColor"
      ...
      <div className="col-span-full   h-24">
        <Link href={`/${props.post.slug}`}>
          ...
          <p className="line-clamp-2">{props.post.description}</p>
        </Link>
