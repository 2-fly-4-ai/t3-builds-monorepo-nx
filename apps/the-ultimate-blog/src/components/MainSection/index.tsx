...
export default function MainSection() {
  ...
  return (
    <main className="border-gray col-span-8 h-full  w-full border-r border-gray-300">
      <div className="mx-16 mb-4 flex flex-col items-center border-b-2 py-4">
        <div className="flex w-full items-center justify-between space-x-4  py-2 ">
          <label
            htmlFor="search"
            className="group flex w-96 items-center justify-center rounded-full border-2 border-gray-300 bg-white p-1 px-2 font-medium focus-within:border-black focus-within:shadow-[1.0px_1.0px_0px_0px_rgba(109,40,217)] focus:border-2  focus:shadow-black"
          >
            <AiOutlineSearch />
            <input
              type="text"
          ...
          <div className="ml-auto flex gap-2 ">
            <div className="my-auto mx-2">My Topics:</div>
            <div>
              <ul className=" flex items-center justify-center space-x-2 text-sm">
                {Array.from({ length: 4 }).map((_, i) => (
                  <li
                    key={i}
                    className="flex cursor-pointer items-center space-x-2 rounded-full border-2  border-gray-300 bg-white p-2 px-3 py-1 font-medium shadow-[1.0px_1.0px_0px_0px_rgba(109,40,217)]  shadow-gray-300 hover:border-black hover:shadow-black"
                  >
                    Design
                  </li>
                ))}
