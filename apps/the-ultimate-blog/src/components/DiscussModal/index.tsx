...
export const WriteFormSchema = z.object({
  ...
  link: z.string().min(100).optional(),
...
export default function PreviewModal({ post }: PreviewModalProps) {
  ...
  // Variables
  const { id, title, link, slug, tags, featuredImage, description } = post;
  ...
  const handleEditPost = trpc.post.editLinkPost.useMutation({
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    defaultValues.link = link; // Replace 'Example HTML' with your actual HTML value
    reset({ ...defaultValues });
